...
def CRFNFL_adp(classifier, traindata, val, ntree=40, niThreshold=11):
    ...
    time_CRF = time.clock() - time_start_crf
    time_start_kmeans = time.clock()
    ...
    for i in range(1, len(kmeans_label)):
        if kmeans_label[i] != kmeans_label[i - 1]:
            tmp_noise = i + 2
    time_Kmeans = time.clock() - time_start_kmeans
...
def crfnfl_adp(classifier, traindata):
    ...
    return train, save_best, save_noise, time_Kmeans, tmp_noise, train_kmeans
